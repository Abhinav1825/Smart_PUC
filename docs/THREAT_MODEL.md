...
| A11 | DoS against backend | Flood `/api/record` | G6 | FastAPI endpoint | Per-IP token-bucket rate limiter (default 120 req/min) as ASGI middleware; JWT required for authority endpoints; API key required for write endpoints. SQLite-persisted rate limits (see `backend/persistence.py`). | Distributed (botnet) DoS still possible; recommend CloudFlare or WAF in front. |
